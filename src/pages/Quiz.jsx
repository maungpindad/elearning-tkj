...
const getScoreMessage = (score) => {
  if (score >= 90)
    return {
      text: "Luar Biasa! 🎉",
      subtext: "Kamu benar-benar menguasai materi ini!",
      color: "text-emerald-600 dark:text-emerald-400",
    };
  if (score >= 80)
    return {
      text: "Sangat Bagus! 👏",
      subtext: "Pemahamanmu sangat baik, pertahankan!",
      color: "text-emerald-600 dark:text-emerald-400",
    };
  if (score >= 70)
    return {
      text: "Bagus! 👍",
      subtext: "Cukup menguasai, tapi masih bisa ditingkatkan.",
      color: "text-amber-600 dark:text-amber-400",
    };
  if (score >= 50)
    return {
      text: "Lumayan 📚",
      subtext: "Beberapa konsep perlu dipelajari ulang.",
      color: "text-amber-600 dark:text-amber-400",
    };
  return {
    text: "Ayo Belajar Lagi! 💪",
    subtext: "Jangan menyerah, ulangi materi dan coba lagi!",
...
const Quiz = () => {
  ...
  const [selectedTopic, setSelectedTopic] = useState(
    INITIAL_STATE.selectedTopic,
  );
  const [selectedDifficulty, setSelectedDifficulty] = useState(
    INITIAL_STATE.selectedDifficulty,
  );
  const [currentQuestionIndex, setCurrentQuestionIndex] = useState(
    INITIAL_STATE.currentQuestionIndex,
  );
  const [selectedAnswer, setSelectedAnswer] = useState(
    INITIAL_STATE.selectedAnswer,
  );
  ...
  const currentQuiz = useMemo(
    () =>
      selectedTopic
        ? quizData.find((q) => q.hardwareId === selectedTopic)
        : null,
    [selectedTopic],
  );
  const questions = useMemo(
    () =>
      !currentQuiz || !selectedDifficulty
        ? []
        : currentQuiz.questions[selectedDifficulty] || [],
    [currentQuiz, selectedDifficulty],
  );
              ...
              <div className="bg-white dark:bg-slate-800 rounded-xl border border-slate-200 dark:border-slate-700 shadow-sm dark:shadow-slate-900/50 p-6 sm:p-8 mb-6">
                ...
                <div className="flex flex-col gap-3">
                  {currentQuestion.options.map((option, idx) => {
                    let optionStyle =
                      "border-slate-200 dark:border-slate-600 bg-white dark:bg-slate-700 hover:border-indigo-300 dark:hover:border-indigo-500 hover:bg-indigo-50/50 dark:hover:bg-indigo-900/20";
                    if (showFeedback) {
                      if (option === currentQuestion.correctAnswer)
                        optionStyle =
                          "border-emerald-400 bg-emerald-50 dark:bg-emerald-900/30 dark:border-emerald-600 ring-1 ring-emerald-200 dark:ring-emerald-700";
                      else if (
                        option === selectedAnswer &&
                        option !== currentQuestion.correctAnswer
                      )
                        optionStyle =
                          "border-red-400 bg-red-50 dark:bg-red-900/30 dark:border-red-600 ring-1 ring-red-200 dark:ring-red-700";
                      else
                        optionStyle =
                          "border-slate-100 dark:border-slate-600 bg-slate-50 dark:bg-slate-700 opacity-50";
    ...
    return (
      ...
      >
        <div className="max-w-lg mx-auto text-center">
          ...
          >
            <h2
              className={`text-2xl font-bold ${getScoreMessage(score).color}`}
            >
              {getScoreMessage(score).text}
            </h2>
            <p className="text-slate-500 dark:text-slate-400 mt-2">
              {getScoreMessage(score).subtext}
            </p>
          </motion.div>
          <motion.div
            initial={{ opacity: 0, y: 10 }}
          ...
          >
            ...
            >
              <RotateCcw className="w-4 h-4" /> Kembali ke Pilihan Kuis
            </button>
            <Link
              to="/dashboard"
